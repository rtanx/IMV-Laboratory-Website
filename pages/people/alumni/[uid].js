                  ...
                  <FaGithub size={24} className="text-gray-900" />
					...
					<div className="mt-8 p-8 rounded-xl bg-pink-100">
						...
						<ul className="list-disc ml-8">
							{Array.isArray(alum.expertise) ? (
								alum.expertise.map((item, index) => <li key={index}>{item}</li>)
							) : Array.isArray(alum.expertise?.expertise) ? (
								alum.expertise.expertise.map((item, index) => <li key={index}>{item}</li>)
							) : (
								<li>{alum.expertise?.expertise}</li>
							)}
						</ul>
					</div>

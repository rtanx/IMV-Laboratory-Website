  ...
  return (
    <header className="sticky top-0 md:-top-12 z-50 flex justify-between items-center h-20 md:h-32 px-4 md:px-8 bg-white border-b border-gray-200">
      <div className="sticky top-0 flex md:justify-between items-center h-20 w-full">
        ...
        <Image src={Logo} width={42} height={42} objectFit="contain" alt="IMV Logo" />
        <h1 className="hidden md:block mr-auto ml-4 text-lg font-medium tracking-wider uppercase">IMV Laboratory</h1>
        <nav>
          <ul
        ...
        <button onClick={() => (document.location.href = "https://webinar.imvlaboratory.com")} className="flex items-center ml-auto md:ml-16 px-4 h-10 rounded-lg bg-blue-500 text-white hover:bg-blue-700 focus:outline-none">
          <VideoCameraIcon className="w-6 h-6" />
          <p className="ml-2 uppercase text-sm font-medium tracking-wide">JOIN WEBINAR</p>
        </button>
      </div>
    </header>
  );

    ...
    <div className="p-4 md:p-8 pb-12 mb-8">
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-8 place-items-center">
        {assistants?.map((assistant, idx) => (
          <div onClick={() => router.push(`/people/${assistant.user_id}`)} key={idx} className="cursor-pointer">
            <Image src={assistant.photoURL} width={100} height={100} objectFit="cover" className="rounded-full pointer-events-none" />
          </div>
        ))}
      </div>
    </div>
